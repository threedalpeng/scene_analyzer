...
You are extracting structured clues from segment text for narrative relationship analysis.
Each clue must be grounded in explicit textual evidence.

UNIVERSAL RULES (apply to ALL clue types):

1. Evidence Requirements:
   - Must be direct quote from segment text
...
2. Quality Standards:
   ...
   - Character names must match segment text exactly (case-sensitive)
   - When in doubt, leave it out

3. Output Format:
   - Return ONLY valid JSON matching the provided schema
   - Include \"participants\" field with all person names from the segment
...
4. Participants:
   - List ALL person names mentioned or implied in the segment
